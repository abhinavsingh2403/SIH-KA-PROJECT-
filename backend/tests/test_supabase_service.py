"""
Unit & Integration Tests for PostgreSQL & Cloud Persistence Service.
"""
...
def test_supabase_service_initialization():
    ...
    assert status['mode'] in ('postgres_direct', 'postgres_cloud_sync', 'cloud', 'local_fallback', 'unconfigured')
...
def test_supabase_flight_and_telemetry_save():
    flight_id = 'test_flight_postgres_001'
...
def test_supabase_alert_save_and_retrieve():
    alert_payload = {
        'alert_id': 'test_alert_postgres_999',
        'flight_id': 'test_flight_postgres_001',
        'timestamp': 120.5,
    ...
    assert any(a['alert_id'] == 'test_alert_postgres_999' for a in alerts)
...
def test_api_supabase_endpoints():
    ...
    res_flights = client.get('/api/database/flights')
    assert res_flights.status_code == 200
    assert isinstance(res_flights.json(), list)

    res_alerts = client.get('/api/database/alerts')
    assert res_alerts.status_code == 200
    assert isinstance(res_alerts.json(), list)

    res_sync = client.post('/api/database/sync-flight/demo_test_sync')
